...
namespace Caffeine.Services
{
    public class StandardCaffeineDecayStrategy : ICaffeineDecayStrategy
    {
        ...
        public TimeSpan TimeToPeak =>
            TimeSpan.FromMinutes(PeakTimeMinutes);

        private const double HalfLifeHours = 5.0;

        public double CalculateActiveCaffeine(
            ...
            DateTime currentTime)
        {
            ...
            {
                return initialAmountMg *
                       (timePassed.TotalMinutes / PeakTimeMinutes);
            }

            double hoursSincePeak =
                (timePassed.TotalMinutes - PeakTimeMinutes) / 60.0;

            return initialAmountMg *
                   Math.Pow(0.5, hoursSincePeak / HalfLifeHours);
        }
    }
}